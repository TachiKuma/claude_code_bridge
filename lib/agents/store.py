...
def _agent_runtime_from_record(record: dict) -> AgentRuntime:
    ...
    return AgentRuntime(
        ...
        backend_impl=record.get('backend_impl'),
        pane_ref=(dict(record.get('pane_ref')) if isinstance(record.get('pane_ref'), dict) else None),
        namespace_ref=(dict(record.get('namespace_ref')) if isinstance(record.get('namespace_ref'), dict) else None),
