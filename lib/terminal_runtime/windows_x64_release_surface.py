...
PROJECTION_RELATIVE_PATH = Path("lib/terminal_runtime/windows_x64_release_surface_projection.json")
SCHEMA_VERSION = 1
...
_ENUMS: dict[str, set[str]] = {
    ...
    "baseline_version_status": {"v8.5.2", "mismatch", "unknown"},
...
def assert_windows_x64_release_surface_projection_fresh(root: str | Path) -> None:
    projection_path = Path(root) / PROJECTION_RELATIVE_PATH
    current = projection_path.read_text(encoding="utf-8")
    expected = canonical_projection_json(default_blocked_projection())
    if current != expected:
...
def windows_x64_release_surface_baseline_version_admission(root: str | Path) -> dict[str, object]:
    ...
    if version == "8.5.2" and package_version == "8.5.2":
        return {
            "status": "ready",
    ...
    return {
        ...
        "reason": f"expected VERSION/package.json 8.5.2, got {version or 'missing'}/{package_version or 'missing'}",
    }


def windows_x64_release_surface_dependency_admission(root: str | Path) -> dict[str, object]:
    root_path = Path(root)
    items_path = root_path / ".codestable" / "roadmap" / "windows-native-herdr-ccb" / "windows-native-herdr-ccb-items.yaml"
    items_text = _read_text(items_path)
    required_items = {
        "windows-x64-v852-baseline-gate": _item_status(items_text, "windows-x64-v852-baseline-gate"),
        "herdr-user-surfaces-parity": _item_status(items_text, "herdr-user-surfaces-parity"),
    }
    not_done = [slug for slug, status in required_items.items() if status != "done"]
    if not_done:
        return _blocked_dependency_admission(f"parent item not done: {', '.join(not_done)}")

    baseline_acceptance = (
        root_path
        / ".codestable"
        / "features"
        / "2026-07-31-windows-x64-v852-baseline-gate"
        / "windows-x64-v852-baseline-gate-acceptance.md"
    )
    surface_acceptance = (
        root_path
        / ".codestable"
        / "features"
        / "2026-07-31-herdr-user-surfaces-parity"
        / "herdr-user-surfaces-parity-acceptance.md"
    )
    for path in (baseline_acceptance, surface_acceptance):
        if not path.exists():
            return _blocked_dependency_admission(f"parent acceptance missing: {_rel(root_path, path)}")
        frontmatter = _frontmatter(path)
        if frontmatter.get("doc_type") != "feature-acceptance" or frontmatter.get("status") != "passed":
            return _blocked_dependency_admission(f"parent acceptance not passed: {_rel(root_path, path)}")
    ...
    surface_evidence = (
        ...
        / "2026-07-31-herdr-user-surfaces-parity"
        / "evidence"
        / "cmd-008-native-windows-surface-transcript.md"
    )
    if not baseline_evidence.exists():
        return _blocked_dependency_admission(f"baseline evidence missing: {_rel(root_path, baseline_evidence)}")
    if not surface_evidence.exists():
        return _blocked_dependency_admission(f"user surface evidence missing: {_rel(root_path, surface_evidence)}")

    baseline_text = _read_text(baseline_acceptance)
    surface_text = _read_text(surface_acceptance)
    if "platform-gate-summary.json" not in baseline_text or "strict-v8.5.2" not in baseline_text:
        return _blocked_dependency_admission("baseline acceptance lacks strict v8.5.2 evidence refs")
    if "CMD-008" not in surface_text or "support_tier_projection" not in surface_text:
        return _blocked_dependency_admission("user surface acceptance lacks CMD-008/support projection evidence refs")

    return {
        "status": "ready",
        "implementation_admission": "admitted",
        "baseline_gate_ref": _rel(root_path, baseline_acceptance),
        "user_surfaces_parity_ref": _rel(root_path, surface_acceptance),
        "baseline_evidence_ref": _rel(root_path, baseline_evidence),
        "user_surface_evidence_ref": _rel(root_path, surface_evidence),
    }
...
def default_blocked_projection(
    *,
    ...
    return {
        ...
        or "Use install.ps1 for source/dev checkout installs and keep release/npm routes diagnostic-only.",
    }
...
def _item_status(items_text: str, slug: str) -> str:
    in_item = False
    for line in items_text.splitlines():
        ...
        if stripped.startswith("- slug:"):
            if in_item:
                return ""
            in_item = stripped.split(":", 1)[1].strip().strip('"') == slug
            continue
        if in_item and stripped.startswith("status:"):
