...
**Implementation:** `3b4f75b4`、本补口提交

**Evidence:** `lib/platforms/windows/herdr/runtime/client.py`、`test/test_herdr_backend_client.py`

**Notes:** 握手缓存现在记录 `runtime_generation`；`HerdrBackend.set_pane_identity()` 将已有
`namespace_epoch` 作为 runtime generation 传入，generation 改变时会重新获取 `server_info()`。
