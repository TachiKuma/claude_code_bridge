...
`lib/platforms/windows/herdr/runtime/events.py`、`lib/ccbd/services/runtime_runtime/refresh.py`

- [ ] `invalidate_handshake()` 或 transient-unavailable 后下一轮必须重读 snapshot
- [ ] generation 改变时丢弃旧 seq 游标和旧 pane 状态
- [ ] snapshot 缺目标 pane 时，目标状态投影为 unknown 或 pane-missing，不回退旧 root summary
- [ ] 重连后的第一批增量事件必须满足当前 binding/generation/seq
- [ ] 测试覆盖：断线重连、generation 切换、pane_id 变更、旧事件延迟到达
