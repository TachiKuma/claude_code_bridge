...
- [ ] 前台启动路径的命令执行通过可注入 runner 完成，默认 runner 保持现有行为
- [ ] 未注入自定义 runner 时，启动行为与改动前完全一致（characterization test 绿）
- [ ] 新 seam 可被测试注入以捕获「探活/spawn」调用而不启动真实 WezTerm 进程
- [ ] 不改变任何用户可见行为，不引入新窗口模型
