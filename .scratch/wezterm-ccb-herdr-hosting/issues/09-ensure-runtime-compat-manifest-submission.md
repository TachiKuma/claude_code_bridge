# 09：ensure_runtime(manifest) CCB 收敛职责 + start/herdr-open 改提交 manifest（Phase 2）

**What to build：** 在 CCB 内实现 `ensure_runtime(manifest, restore_token)` 运行时收敛职责——内部仍可调用
既有 create/ensure/pane 操作，但对外只暴露「声明拓扑、收敛 runtime」的语义。`handle_start` 与
`handle_herdr_open` 改为提交 manifest，不再直接触发运行时证据注入。bootstrap 降级为 CCB 内部实现细节：
...
**Notes:** ADR 0002 已将该路径固化为 CCB 的长期运行时收敛职责；内部仍复用既有
namespace/materializer 与旧字段兼容。

- [x] `ensure_runtime(manifest, restore_token)` CCB 收敛职责落地，内部复用既有操作
