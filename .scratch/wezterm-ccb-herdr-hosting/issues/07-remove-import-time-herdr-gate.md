...
- [ ] 配置检查等 introspection 命令不触发任何 Herdr 探测
- [ ] 需要运行时的命令在 operation-time 检查 Herdr 可用性，返回结构化错误
- [ ] 明确选择 Herdr 且不可用时 fail-closed（清晰错误，非含糊行为）
- [ ] 旧路径仍 fail-closed（characterization test 佐证）
