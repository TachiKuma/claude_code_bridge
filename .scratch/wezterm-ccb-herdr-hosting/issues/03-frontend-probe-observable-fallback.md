...
- [ ] 有可用 WezTerm GUI mux 时，agent tab 在其中开出
- [ ] 无 mux 时**触发可观测回退**到 Herdr 独立窗口（不再静默），回退原因被记录
- [ ] spawn 失败（返回码非 0）被正确判定，不被丢弃
- [ ] 无 WezTerm 二进制时也走可观测回退
- [ ] `frontend.mux_available` 在「有 mux / 无 mux / 无二进制」三态下取值正确
- [ ] 本轮不引入新窗口寻址（维持现有窗口模型）
