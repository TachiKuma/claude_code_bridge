...
- [ ] Ready Gate 定义：agent 达到 Agent Ready 须同时满足三个条件：
...
- [ ] Ready Gate 不把「运行时完成」（Herdr `done`）、「进程存在」、「前台可见」或「provider banner 已出现」视为达到 Agent Ready。
- [ ] Ready Gate 消费运行时事实（Herdr snapshot、health check 结果、binding 校验结果），但不把运行时事实升级为业务完成判定。
- [ ] 并发上限设计：
...
- [ ] Partial ready 状态可观测：用户能看到哪些 agent 已 ready、哪些仍在收敛、哪些失败。
- [ ] 启动度量记录：总 ready 时间、每个 agent 的 ready 时间、并发等待时间、各阶段耗时分布。
- [ ] 结果汇总：`start` 命令返回 `AllReadyResult`，包含所有 agent 的最终状态、时间度量和失败详情。
