...
- The current runner reaches Herdr CLI schema/status/workspace/pane commands. `pane run` plus `pane wait-output` observes the sentinel. Restore Capability Matrix v2 shows server restart layout identity restore passes, while process continuity and marker output history are unsupported. UI detach/reattach remains unexercised from the non-Herdr harness and is tracked as `herdr-ui-detach-reattach-harness`. Partial evidence is valid spike output for this feature; downstream adapter work may continue only with layout-only restart restore and must not treat the gaps as supported.
