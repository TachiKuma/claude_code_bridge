...
- `herdr-v0.8-version`: exit=0, elapsed=19ms
- `herdr-v0.8-api-help`: exit=0, elapsed=19ms
- `herdr-v0.8-status-server`: exit=0, elapsed=18ms
- `herdr-v0.8-plugin-list`: exit=0, elapsed=19ms
- `herdr-v0.8-help`: exit=0, elapsed=18ms
- `herdr-v0.8-session-list`: exit=0, elapsed=18ms
- `herdr-v0.8-status`: exit=0, elapsed=18ms

## Integration Readiness Assessment

| Capability | v0.7.5 Status | v0.8.0 Probe Result | C2 Impact |
|---|---|---|---|
| Session reporting format | `herdr status server --json` | see `herdr-v0.8-status-server.stdout.txt` | CCB鈫扝erdr evidence path |
