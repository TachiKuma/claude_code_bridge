...
```bash
python3 <cs-onboard skill 目录>/tools/codestable-goal-consistency-gate.py --roadmap .codestable/roadmap/windows-rmux-native-backend
```
...
写 `.codestable/roadmap/windows-rmux-native-backend/goal-audit.md`：
...
# windows-rmux-native-backend Goal 最终审计
...
同时写 `.codestable/roadmap/windows-rmux-native-backend/goal-evidence-summary.md` 或在 goal-audit 第 7 节等价内嵌：
