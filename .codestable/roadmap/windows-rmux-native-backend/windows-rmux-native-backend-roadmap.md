...
### Target-Platform Evidence Policy（2026-07-20 owner decision）

本轮 goal 的核心平台是 native Windows，当前目标机没有 `socket.AF_UNIX` 是目标环境事实，不是需要绕到 Unix/WSL 补证据的异常。`ccbd-control-plane-transport-seam` 可以把真实 Unix `AF_UNIX` bootstrap/lifecycle/stale-cleanup 证据作为 compatibility residual；它不单独阻塞 `windows-rmux-native-working`。Windows 控制面核心证据由 `ccbd-windows-tcp-loopback-transport` 与 `ccbd-windows-full-chain-smoke` 提供。

这条 policy 不豁免 native Windows blocker。任何会阻断 Windows collection、start、ping、doctor、ask、kill 或 full-chain smoke 的问题仍是核心缺口，例如 `mobile_gateway.terminal -> import fcntl` 顶层导入、runtime accelerator `AF_UNIX` AttributeError、Windows pid liveness 误判等。它们必须在对应 feature 中修复，或由 owner 对具体缺口另行记录可接受决策。
...
**证据边界补充（2026-07-20）**：`ccbd-control-plane-transport-seam` 的 Unix adapter 真实 `AF_UNIX` 主机证据在 native Windows milestone 中不作为 blocking core；缺口必须保留在 QA / acceptance residual risk，并由 `ccbd-windows-tcp-loopback-transport` 继续补足 Windows 控制面真实证据。Windows `fcntl` collection baseline 与其他 native blocker 不属于该 residual。
...
| Unix `AF_UNIX` adapter 兼容性不被 seam 破坏（非 Windows milestone core） | `ccbd-control-plane-transport-seam` | Unix-capable host/CI 真实 socket tests；native Windows 可记录 skipped compatibility residual | pytest / residual risk | no |
...
- **2026-07-20 evidence decision**：owner 明确当前目标是纯 native Windows 环境，`socket.AF_UNIX` 缺失符合目标平台事实。Goal / QA / audit 不应要求 Unix/WSL/CI `AF_UNIX` 证据作为 `windows-rmux-native-working` 恢复条件；真实 Unix adapter 证据保留为 compatibility residual。`mobile_gateway.terminal -> import fcntl` 属 Windows collection blocker，仍需修复或单独接受。
