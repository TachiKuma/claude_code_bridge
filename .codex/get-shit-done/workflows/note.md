...
- **Global scope**: `D:/Python/GitHub/claude_code_bridge/.codex/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/Python/GitHub/claude_code_bridge/.codex/notes/`)
...
2. Glob `D:/Python/GitHub/claude_code_bridge/.codex/notes/*.md` (if directory exists) — global notes
...
Global (D:/Python/GitHub/claude_code_bridge/.codex/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/Python/GitHub/claude_code_bridge/.codex/notes/` — works in any directory
