...
```bash
TRANSITION=$(node "E:/GitHub开源项目/TachiKuma/claude_code_bridge/.gemini/get-shit-done/bin/gsd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "E:/GitHub开源项目/TachiKuma/claude_code_bridge/.gemini/get-shit-done/bin/gsd-tools.cjs" progress bar --raw)
```
...
if [ -n "$GSD_WORKSTREAM" ]; then
  WS_LIST=$(node "E:/GitHub开源项目/TachiKuma/claude_code_bridge/.gemini/get-shit-done/bin/gsd-tools.cjs" workstream list --raw)
fi
```
...
```bash
node "E:/GitHub开源项目/TachiKuma/claude_code_bridge/.gemini/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false
```
